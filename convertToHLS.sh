...
for video in "${VIDEO_FOLDER}"/*.mp4; do
  ...
  for i in "${!BITRATES[@]}"; do
    ...
    ffmpeg -i "${video}" -c:v libx264 -b:v "${bitrate}" -s "${resolution}" -profile:v main -level 3.0 -preset medium -g 30 -hls_time 0.1 -hls_list_size 0 -hls_segment_filename "${video_folder}/${filename}_${bitrate}_${resolution}_%03d.ts" "${video_folder}/${output_file}"
